...
public:
    ...
    tuples& operator=(const tuples& other) {
        if (&other != this)
            memcpy(this->data, &other.data, SIZE * sizeof(TYPE));
        return *this;
    }
    
    tuples& operator=(const TYPE array[SIZE]) {
        memcpy(data, array, SIZE * sizeof(TYPE));
        return *this;
    }
...
template <typename TYPE, unsigned SIZE>
std::ostream& operator<<(std::ostream &out, const tuples<TYPE,SIZE> x)
{
    out << '[';
